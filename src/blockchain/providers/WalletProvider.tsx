...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Retry check after a delay (MetaMask might load after page)
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      console.log("[MetaMask] Current chain ID:", currentChainId);

      // Check if on XRP EVM Testnet
      if (currentChainId !== XRP_EVM_CONFIG.chainId) {
        console.log(`[MetaMask] Wrong network (${currentChainId}), switching to XRP EVM Testnet (${XRP_EVM_CONFIG.chainId})...`);
        ...
        try {
          ...
          console.log("[MetaMask] Successfully switched to XRP EVM Testnet");
        } catch (switchError: any) {
          console.log("[MetaMask] Switch error:", switchError);
          // Chain not added, add it
          if (switchError.code === 4902) {
            console.log("[MetaMask] Adding XRP EVM Testnet to MetaMask...");
            await window.ethereum.request({
            ...
            console.log("[MetaMask] Successfully added XRP EVM Testnet");
          } else {
            throw switchError;
          }
        }
